...
既存のNode.js + Express APIを、DynamoDBからAmazon RDS for PostgreSQLへ移行した記録。**移行は完了しており、DynamoDB版のコード・AWS上のテーブルは削除済み。**
...
- DynamoDB版のコード（`src/dynamo.js`、`deploy/create-tables.sh`、`deploy/deploy.sh`、`scripts/seed-dynamodb.js`）とAWS上のDynamoDBテーブルは削除済み（移行完了のため）
...
- DynamoDB版のコード・AWS上のテーブルは削除済み。ロールバックが必要な場合はgit履歴（`main`のRDS移行前のコミット）から復元する
- Python版Lambdaは比較・検証用として当面削除しない
